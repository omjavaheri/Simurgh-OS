...
// build.rs — fs-native
//
// Emits the linker-script argument for `fs-native-bin`'s own `[[bin]]`
// link step. Mirrors `subsystems/device-manager/build.rs` exactly (see
// that file's own doc comment for the full rationale — Cargo only
...
// build of this crate's library — `fs-native` is also a normal `rlib`
