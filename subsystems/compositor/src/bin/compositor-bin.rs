...
//! `kernel_arch_glue::compositor_demo_start` loads this ELF into a
//! fresh, isolated process via `spawn_process_from_elf`.
//!
//! Architecture reference: 03-Kernel-Subsystems-Layer.md §2.4/§5.4.2 (a
//! real, isolated Compositor Service).
//!
...
//! All real logic stays in `compositor::subsystem_entry::subsystem_main`.
//! Every byte of this compiled output is mapped `U=1` (see `kernel_
//! arch_glue::spawn_process_from_elf`'s own per-segment permission
//! derivation), so calling ordinary (non-inlined) functions from
//! anywhere in this crate's own dependency graph — `ipc_protocol::
//! codec`, `compositor::Compositor` — is completely safe.
//!
...
// ----------------------------------------------------------------------------
// A minimal global allocator — `compositor::Compositor` uses `alloc`
// (BTreeMap) for its own surface table, and this binary is `#![no_std]`
...
pub extern "C" fn _start() -> ! {
    compositor::subsystem_entry::subsystem_main()
}
