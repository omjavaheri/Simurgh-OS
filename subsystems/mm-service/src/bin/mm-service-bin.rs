...
//! `kernel_arch_glue::mm_demo_start` loads this ELF into a fresh,
//! isolated process via `spawn_process_from_elf`.
//!
//! Architecture reference: 03-Kernel-Subsystems-Layer.md §2.5 (a real,
//! isolated memory-policy service).
//!
...
//! All real logic stays in `mm_service::subsystem_entry::subsystem_main`.
...
//! codec`, `mm_service::MemRegistry` — is completely safe.
//!
//! Safety/invariants: no stack/`.bss` setup of its own — see
//! `compositor-bin`'s own doc comment for why `_start` is reached with
//! both already valid.
//! ============================================================================
#![no_std]
#![no_main]
...
// ----------------------------------------------------------------------------
// A minimal global allocator — `mm_service::MemRegistry` uses `alloc`
// (Vec) for its own registered-process table, and this binary is
// `#![no_std]` with no heap of its own otherwise. Same bump-allocator
// shape every other subsystem-bin already uses for the SAME reason.
// ----------------------------------------------------------------------------
...
pub extern "C" fn _start() -> ! {
    mm_service::subsystem_entry::subsystem_main()
}

/// This binary's own, mandatory panic handler — see `compositor-bin`'s
