...
//! "subsystems as processes" packaging, mirroring `compositor-bin`
//! exactly (see that crate's own doc comment for the full rationale).
//! `kernel_arch_glue::security_broker_intermediary_demo_start` loads this
//! ELF into a fresh, isolated process via `spawn_process_from_elf`.
//!
//! Architecture reference: Issue #28 (the CapGrant/CapRevoke
...
//! All real logic stays in `security_broker_intermediary::
//! subsystem_entry::subsystem_main`. Every byte of this compiled output
//! is mapped `U=1` (see `kernel_arch_glue::spawn_process_from_elf`'s own
//! per-segment permission derivation), so calling ordinary (non-inlined)
//! functions from anywhere in this crate's own dependency graph
//! (`ipc_protocol::codec`, `alloc`) is completely safe here.
//!
//! Safety/invariants: no stack/`.bss` setup of its own — see `fs-native-
//! bin`'s own doc comment for why `_start` is reached with both already
//! valid.
//! ============================================================================
#![no_std]
#![no_main]
...
// ----------------------------------------------------------------------------
// A minimal global allocator — `security_broker_intermediary::
// Intermediary` uses `alloc` (`BTreeMap`) for its own target-service
// table, and this binary is `#![no_std]` with no heap of its own
// otherwise. Same bump-allocator shape every other subsystem-bin already
...
pub extern "C" fn _start() -> ! {
    security_broker_intermediary::subsystem_entry::subsystem_main()
}

/// This binary's own, mandatory panic handler — see `fs-native-bin`'s
/// own doc comment on why a separately-linked `[[bin]]` needs one where
/// the library crate's own modules did not.
#[panic_handler]
