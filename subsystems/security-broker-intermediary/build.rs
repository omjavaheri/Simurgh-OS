// ============================================================================
// build.rs — security-broker-intermediary
//
// Emits the linker-script argument for `security-broker-intermediary-bin`'s
// own `[[bin]]` link step. Mirrors `subsystems/compositor/build.rs` exactly
// (see that file's own doc comment for the full rationale — Cargo only
// honors `cargo:rustc-link-arg*` from the crate that actually produces the
// `[[bin]]`).
//
// For any `target_arch` outside the three below (including a plain host
// build of this crate's library — this crate is also a normal `rlib` with
