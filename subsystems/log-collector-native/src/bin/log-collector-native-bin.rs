//! ============================================================================
//! log-collector-native-bin
//!
//! Purpose: this crate's REAL, separately-built process image — the
//! "subsystems as processes" packaging, mirroring `mm-service-bin`
//! exactly (see that crate's own doc comment for the full rationale).
//! `kernel_arch_glue::log_collector_demo_start` loads this ELF into a
//! fresh, isolated process via `spawn_process_from_elf`.
//!
//! Architecture reference: 04-System-Services-Policy-Layer-v2.md §2.2
//! (Log Collector, layer 3 — the mechanism half `simurgh-diagnostics`'s
//! own `diagnostics-manager` process talks to as the policy half).
//!
...
//! All real logic stays in `log_collector_native::subsystem_entry::
//! subsystem_main`. Every byte of this compiled output is mapped `U=1`
//! (see `kernel_arch_glue::spawn_process_from_elf`'s own per-segment
//! permission derivation), so calling ordinary (non-inlined) functions
//! from anywhere in this crate's own dependency graph is completely
//! safe.
//!
//! Safety/invariants: no stack/`.bss` setup of its own — see
//! `mm-service-bin`'s own doc comment for why `_start` is reached with
//! both already valid.
//! ============================================================================
#![no_std]
#![no_main]
...
// ----------------------------------------------------------------------------
// A minimal global allocator — this process's own event queue (`alloc::
// collections::VecDeque`) needs a heap, and this binary is `#![no_std]`
// with no heap of its own otherwise. Same bump-allocator shape every
// other subsystem-bin already uses for the SAME reason.
// ----------------------------------------------------------------------------
...
pub extern "C" fn _start() -> ! {
    log_collector_native::subsystem_entry::subsystem_main()
}

/// This binary's own, mandatory panic handler — see `mm-service-bin`'s
